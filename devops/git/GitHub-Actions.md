...
- **Fija las acciones de terceros a un commit, no a una etiqueta** — `uses: actions/checkout@v4` confía en que esa etiqueta siga apuntando a código honesto; si la cuenta del autor se ve comprometida, la etiqueta puede moverse a código malicioso que se ejecutará con acceso a tus secretos. La forma robusta es fijar el hash completo: `uses: actions/checkout@11bd719... # v4.2.2`. Ha habido ataques reales de cadena de suministro que explotaron justo esto.
- **Recorta los permisos del `GITHUB_TOKEN`** — cada workflow recibe un token automático que, por defecto histórico, puede tener más poderes de los que necesita. Declara lo mínimo al inicio del archivo:
  ...
  Así, aunque un paso malicioso o un test comprometido intente escribir en el repo o crear releases, no podrá.
- **`concurrency` para cancelar ejecuciones obsoletas** — si subes tres commits seguidos a un PR, por defecto corren tres workflows completos aunque solo importa el último. Con `concurrency: { group: ci-${{ github.ref }}, cancel-in-progress: true }`, cada push cancela la ejecución anterior de esa rama: colas más cortas y menos minutos consumidos.
- **Pon siempre `timeout-minutes` a los jobs** — el límite por defecto es de varias horas: un test colgado puede dejar el runner ocupado (y facturando, en repos privados) toda la tarde. Un `timeout-minutes: 15` en cada job convierte los cuelgues en fallos rápidos y visibles.
- **Cachea las dependencias** — instalar `node_modules` desde cero en cada ejecución puede ser la mitad del tiempo total del workflow. Acciones como `actions/setup-node` con `cache: npm` (o `actions/cache` a mano) reutilizan lo descargado entre ejecuciones; es la optimización con mejor relación esfuerzo/beneficio de todo el CI.
