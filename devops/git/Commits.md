...
- **`git commit --amend` para el "ay, se me olvidó"** — si justo después de commitear ves una errata o un archivo que faltaba, prepáralo y ejecuta `git commit --amend`: reescribe el último commit en lugar de añadir uno de "arreglito". Ojo: cambia el identificador del commit, así que solo vale mientras **no lo hayas subido** al remoto.
- **Escribe el mensaje con asunto y cuerpo** — primera línea corta (≈50 caracteres, en imperativo: "Corrige el cálculo del IVA"), una línea en blanco, y debajo el *porqué*: qué alternativas descartaste, qué caso raro cubre. Herramientas como `git log --oneline` solo muestran la primera línea, por eso la separación en blanco no es estética: es parte del formato.
- **Adopta una convención de mensajes como Conventional Commits** — prefijos del tipo `feat:`, `fix:`, `refactor:` hacen el historial filtrable y permiten automatizar cosas como generar el changelog o decidir el número de versión. Lo valioso no es el prefijo en sí, sino que obliga a que cada commit sea *una sola cosa*: si no sabes qué prefijo ponerle, el commit mezcla demasiado.
- **Escribe el mensaje mirando el diff: `git commit -v`** — abre el editor con el diff completo debajo del mensaje. Además de ayudarte a describir lo que de verdad cambió, funciona como última revisión: más de un archivo colado se ha detectado gracias a esa vista.
