...
Playwright ocupa la **cima de la pirámide de tests** (ver [Tipos de tests](../testing-dotnet/Tipos-de-tests.md)): por encima de los unitarios (rápidos y numerosos, con [Vitest](../../desarrollo-web/frontend-react/Vitest.md) en el frontend, que corren en milisegundos) y de los de integración (piezas reales juntas, con [Testcontainers](../testing-dotnet/Testcontainers.md) dando una base de datos de verdad). Arrancar un navegador y todo el stack cuesta segundos, no milisegundos, así que son pocos, lentos y caros de mantener — pero son los únicos que prueban el sistema **entero**, y los únicos capaces de decir *que* algo falla en la integración real; localizar *dónde* sigue siendo trabajo de los tests de integración.
...
No pruebes *todo* con E2E: es lento y frágil, y no mide cuántos usuarios simultáneos aguanta un sistema (para eso hacen falta tests de carga, no E2E). Cubre el puñado de caminos que, si se rompen, dejan la aplicación inservible.
...
## Locators: apunta a lo que ve el usuario, no al HTML
...
## Page Objects: encapsular una pantalla
...
## Fixtures: preparar el contexto de cada test
...
## Trazas y artefactos de depuración
...
- [Documentación de Playwright](https://playwright.dev/docs/intro) — la referencia completa: locators, aserciones, configuración de proyectos y la API entera de `page`.
- [Best Practices de Playwright](https://playwright.dev/docs/best-practices) — la guía oficial de qué locators usar, cómo aislar tests y cuándo (no) usar esperas manuales; el origen de buena parte de la sección anterior.
