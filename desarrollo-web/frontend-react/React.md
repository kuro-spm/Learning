...
Aparece en cualquier aplicación donde el backend (en C#/.NET, o cualquier otro lenguaje) expone una API REST y una interfaz web necesita consumir esos datos: un dashboard de ventas, un panel de administración, una app de gestión de tareas.

La separación es clara: el backend gestiona datos y lógica de negocio; React gestiona lo que el usuario ve e interactúa.
...
```tsx
<ProductCard title="Zapatillas running" />
```
...
*En resumen: React es el motor de la interfaz en cualquier aplicación con backend propio; convierte los datos que devuelve tu API en pantallas interactivas, organizando todo en componentes reutilizables y actualizando la UI automáticamente cuando los datos cambian.*
