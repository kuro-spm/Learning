...
Aparece en cualquier proyecto frontend con React (u otros frameworks) que necesite un entorno de desarrollo rápido y un build de producción optimizado. Cumple dos roles:

1. **Dev server**: cuando ejecutas `npm run dev`, Vite levanta el frontend en `localhost:5173` y puede conectar con una API backend en otro puerto (por ejemplo, una API en C#/.NET).
2. **Build de producción**: cuando ejecutas `npm run build`, Vite genera los archivos estáticos optimizados en `/dist`, que luego el backend puede servir o desplegar por separado.
