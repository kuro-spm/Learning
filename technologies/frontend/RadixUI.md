...
En **EcoWaveProjectManagement**, Radix UI no se usa directamente en la mayoría de los casos: actúa como la capa inferior de **shadcn/ui**. Cuando usas un `<Dialog>`, un `<DropdownMenu>` o un `<Select>` de shadcn/ui, por debajo está un componente Radix UI gestionando toda la lógica.
...
- **No reemplaza a shadcn/ui en este proyecto.** shadcn/ui ya envuelve Radix con estilos y convenciones; usa shadcn/ui como primera opcion.
- **No gestiona datos ni estado de aplicacion.** Solo gestiona el estado de interaccion UI (abierto/cerrado, seleccionado, etc.).

---

*En resumen: Radix UI es la fontaneria accesible que hay detras de los componentes de shadcn/ui — no la veras directamente en el dia a dia del proyecto, pero es la razon por la que los modales, menus y selects funcionan bien para todos los usuarios.*
