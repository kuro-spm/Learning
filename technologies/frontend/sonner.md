...
En **EcoWaveProjectManagement**, Sonner se usa para dar feedback inmediato al usuario tras operaciones que llaman a la API .NET: crear un proyecto, actualizar el estado de una tarea, o cuando una petición falla. El `<Toaster />` está montado una sola vez en el componente raíz (`App.tsx`), y desde cualquier punto de la app se disparan toasts con la función `toast()`.
...
```tsx
toast.promise(crearProyecto(datos), {
...
- **No reemplaza los modales de confirmación** — para acciones destructivas (borrar un proyecto), sigue usando un diálogo explícito.
