...
En **EcoWaveProjectManagement**, el frontend React está escrito en TypeScript. `typescript-eslint` (v8.59.2) se configura en el archivo `eslint.config.js` (o `.eslintrc`) del directorio `frontend/` y se ejecuta:
...
async function cargarProyectos() {
  fetchProyectos(); // Promise ignorada — el compilador no se queja, el linter sí
}
```
