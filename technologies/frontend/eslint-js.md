...
En **EcoWaveProjectManagement**, `@eslint/js` se usa como base de la configuración de ESLint del frontend React. Se referencia en el archivo `eslint.config.js` (o `eslint.config.mjs`) en la raíz del proyecto frontend y define las reglas mínimas que se aplican a todos los archivos `.js` y `.jsx`.
