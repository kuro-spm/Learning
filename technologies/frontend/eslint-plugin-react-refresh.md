...
En **EcoWaveProjectManagement**, el frontend en React usa Vite como bundler, que incluye Fast Refresh por defecto. Este plugin está configurado en ESLint para marcar automáticamente los archivos del frontend (`/frontend/src/**`) que rompan la compatibilidad con Fast Refresh, evitando recargas completas inesperadas durante el desarrollo.
...
export const MAX_PROJECTS = 10; // esto rompe Fast Refresh
export default function ProjectCard() {
  return <div>...</div>;
}
```
...
```tsx
// utils/projectConstants.ts
export const MAX_PROJECTS = 10;

// components/ProjectCard.tsx
import { MAX_PROJECTS } from '../utils/projectConstants';
export default function ProjectCard() { ... }
```
