...
En **EcoWaveProjectManagement**, TypeScript es la base de todo el frontend React. Cada componente, hook, servicio de API y modelo de datos está escrito en `.ts` o `.tsx`. El compilador de TypeScript (`tsc`) forma parte del proceso de build: si el código no compila, el build falla.

También actúa como contrato entre el frontend y el backend C#/.NET: los tipos que describes en TypeScript deben coincidir con los DTOs que expone la API.
...
```ts
const projectName: string = "EcoWave";
