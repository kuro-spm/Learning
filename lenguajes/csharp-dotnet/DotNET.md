...
.NET 10 es la base de cualquier API backend construida en C#: una tienda online, un sistema de gestión de tareas o un backend de facturación se compilan y corren sobre este runtime. El servidor HTTP, la serialización JSON, la inyección de dependencias y la capa de acceso a datos dependen directamente de las APIs nativas de .NET 10.
...
app.MapGet("/productos", () => Results.Ok(new[] { "PROD-001", "PROD-002" }));
...
No necesitas contenedores externos como Autofac; el DI container de .NET suele ser suficiente para la mayoría de proyectos.
...
*En resumen: .NET 10 es el motor que hace correr todo el backend de una aplicación C#. No interactuas con él directamente en el día a día, pero entender su ciclo de vida, su sistema de configuración y su DI container te ahorrará muchas horas de depuración.*
