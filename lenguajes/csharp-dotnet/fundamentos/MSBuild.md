...
En cualquier solución .NET con varios proyectos —por ejemplo, una tienda online dividida en módulos de Catálogo, Pedidos y Facturación— cada uno tiene su propio `.csproj`. MSBuild es responsable de:

- Compilar cada proyecto de forma independiente o conjunta desde la solución `.sln`
- Gestionar referencias entre proyectos (por ejemplo, si el módulo de Pedidos depende de un proyecto de infraestructura compartido)
- Ejecutar tareas pre/post-build, como copiar archivos estáticos o lanzar migraciones de EF Core en CI/CD

```bash
# Compilar toda la solución
dotnet build MiTienda.sln --configuration Release

# Compilar solo el módulo de Catálogo
dotnet build src/Catalogo/Catalogo.csproj
```
...
`Debug` y `Release` son configuraciones predefinidas que alteran optimizaciones y símbolos. Puedes añadir configuraciones propias para entornos específicos.

**5. MSBuild en CI/CD**

En un pipeline de integración continua, `dotnet publish` invoca MSBuild para generar el artefacto desplegable. Entender qué target ejecuta cada comando ayuda a depurar fallos de pipeline.
...
*En resumen: MSBuild es el motor invisible que convierte tu código C# en software ejecutable; los archivos `.csproj` son su configuración, y entenderlos te da control real sobre cómo se construye y publica cada proyecto de tu solución.*
